...
export default function PropertyReportPage() {
  ...
  const toggleStatus = async (id: string) => {
    ...
    const nextIndex =
      (statusOrder.indexOf(currentReport.status) + 1) % statusOrder.length;
    ...
    if (!error) {
      setReports(
        reports.map((r) => (r.id === id ? { ...r, status: nextStatus } : r)),
      );
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pt-8 pb-12 transition-colors relative">
      {/* Main Container */}
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y dark:divide-slate-700 text-slate-700 dark:text-slate-200">
              {reports.map((report) => (
                ...
                >
                  <td className="p-4 w-40 font-mono text-sm">
                  ...
                  <td className="p-4 w-40 text-sm">
                    ...
                    >
                      <span
                        className={`w-2 h-2 rounded-full ${report.status === "Open" ? "bg-blue-500" : report.status === "In Progress" ? "bg-orange-500" : "bg-green-500"}`}
                      ></span>
                      {report.status}
                    </button>
                  </td>
                  <td className="p-4 w-40 text-sm text-slate-400">
                    {report.date}
                  </td>
